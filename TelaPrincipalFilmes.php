...
<head>
    ...
    <script>
        function excluirFilme(codFilme) {
            var confirmacao = confirm("Deseja realmente excluir este filme?");
        ...
        function editarFilme(codFilme) {
            // Crie uma URL com o código do livro para a página de edição
            var url = "editarFilme.php?codFilme=" + codFilme;
...
<body>
    ...
    <div class="div-botao">
        <a href="CadastroFilmesUsuario.php"><button class="cadastro-botao">Cadastrar</button></a>
    </div>
    <div class="table-container">
        <table class="tabela-scroll">
            ...
            <tbody>
                ...
                $sql = "SELECT l.*, c.nomeConteudo, c.aprovado FROM filme l
            ...
            <td>
                <button class='editar-botao' id='alterar' onclick='editarFilme(" . $row["codFilme"] . ")'></button>
                <button class='excluir-botao' id='excluir' onclick='excluirFilme(" . (isset($row["codFilme"]) ? htmlspecialchars($row["codFilme"]) : 0) . ", this)'></button>
            </td>
          </tr>";
                    }
                } else {
